...
GameObject* World::GetLetShape(cpShape *Shape) {
    for(int i = 0; i < level->size(); i++)
    {
...
void World::DeleteObject(GameObject *obj) {
    if(obj->myBody != nullptr && obj->myBody != cpSpaceGetStaticBody(mSpace))
        cpSpaceRemoveBody(mSpace, obj->myBody);

    if(obj->myShape != nullptr)
        cpSpaceRemoveShape(mSpace, obj->myShape);
